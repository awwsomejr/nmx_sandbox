...
def main():
  ...
  for i in nmxs:
    ...
    try:
        for d in deviceIDs:
            devId = d['DeviceInfo']['ID']
            serviceList = get_data('/api/Topology/v2/Devices/' + devId + '/ServiceList', index, token)
            d['serviceList'] = serviceList
            d['nmxIPAddress'] = index
            data = (json.dumps(d, indent=2, sort_keys=True))

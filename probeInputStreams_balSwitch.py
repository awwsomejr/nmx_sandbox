...
def main():
 ...
 #          if (d['DeviceInfo']['DeviceType']) in 'Electra X': 
 #               for i in range(0, len(d['PortList'])):
 #                   if (d['PortList'][i]['Name']) in 'GbE 6 Rx':
 #                           p6 = d['PortList'][i]['ID']
 #                           d5 = d['DeviceInfo']['ID']
 #                           #print (d5 + ',' + p6)
 #                           inputSources = get_data('/api/Services/v2/InputSources?device=' + d5 + '&port=' + p6, index, token)
 #                           print(inputSources)
 #                           if inputSources:
 #                               data = (json.dumps(inputSources, indent=2, sort_keys=True))
 #                               myList = json.loads(data)
 ...
 #          pass
    ...
    try:
        for d in deviceIDs:
           if (d['DeviceInfo']['DeviceType']) in 'N+M Balanced Switch': 
                for i in range(0, len(d['PortList'])):
                            pt = d['PortList'][i]['ID']
                            d5 = d['DeviceInfo']['ID']
                            #print (d5 + ',' + pt)
                            inputSources = get_data('/api/Services/v2/InputSources?device=' + d5 + '&port=' + pt, index, token)
                            #print(inputSources)
                            if inputSources:
                                data = (json.dumps(inputSources, indent=2, sort_keys=True))
                                myList = json.loads(data)

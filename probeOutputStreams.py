        ...
        for d in deviceIDs:
           if (d['DeviceInfo']['DeviceType']) in 'Electra X': 
                for i in range(0, len(d['PortList'])):
                    if (d['PortList'][i]['Name']) in 'GbE 7 Tx':
                            p7 = d['PortList'][i]['ID']
                            d5 = d['DeviceInfo']['ID']
                            #print (d5 + ',' + p7)
                            outputSources = get_data('/api/Services/v2/OutputTransports?device=' + d5 + '&port=' + p7, index, token)
                            #print(outputSources)
                            if outputSources:
                                data = (json.dumps(outputSources, indent=2, sort_keys=True))
